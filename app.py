...
def thai_date_picker(label, key_prefix, default_date=None, start_year_th=None):
    ...
    with c3:
        if start_year_th:
            ...
            end_year = max(current_year_th, start_year_th) + 3
            year_list = list(range(start_year_th, end_year))
        else:
            # กรณีทั่วไป (ย้อนหลัง 2 ปี ถึง อนาคต 3 ปี)
            year_list = list(range(current_year_th - 2, current_year_th + 3))
    ...
    try:
        ...
        if not worksheet.get_values("A1"):
            headers = [
                ...
                "ScreeningDate", "VisitNo", "Dept", "RightEar", "LeftEar", 
...
with tab1:
    with st.container():
        ...
        with st.form("entry_form", clear_on_submit=False):
            ...
            c4, c5 = st.columns(2)
            with c4: 
                # *** แก้ไข: วันที่คัดกรอง เริ่มปี 2567 เป็นต้นไป ***
                screen_date = thai_date_picker("วันที่คัดกรอง (ด/ว/ป)", "screen_picker", default_date=date.today(), start_year_th=2567)
            ...
            if submit_btn:
                if not hn or not citizen or not name or not recorder:
                ...
                elif screen_date is None or (has_appt and appt_date is None):
                    st.error("⚠️ วันที่ระบุไม่ถูกต้อง")
                else:
                    timestamp = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
                    screen_date_str = screen_date.strftime("%d/%m/%Y")
                    appt_str = appt_date.strftime("%d/%m/%Y") if appt_date else "-"
                    
                    new_row = [
                        timestamp, hn, citizen, name, gender, 
                        screen_date_str, 
                    ...
                    try:
                        cell_hn = None
                        cell_id = None
                        try: cell_hn = ws.find(hn, in_column=2)
                        except: pass
                        try: cell_id = ws.find(citizen, in_column=3)
                        except: pass
...
with tab2:
    ...
    if not df.empty and 'Summary' in df.columns:
        ...
        with c_table:
            # เพิ่ม ScreeningDate เข้าไปในตารางด้วย
            cols_possible = ['Timestamp', 'HN', 'Name', 'Summary', 'ScreeningDate', 'ApptDate']
            valid_cols = [c for c in cols_possible if c in df.columns]
